...
from models.multi_scale_rnn_vae import MultiScaleRNNVAE
from models.loss_functions import vae_loss
...
def train(cfg, latent_dir, save_dir):
    ...
    for epoch in range(cfg.epochs):
        ...
        if (epoch + 1) % 25 == 0:
            for z in loader:
                outputs_recon = model(z, teacher_forcing=False)
            ...
            recon = codec.decode(output_latent)
            orig = codec.decode(z)
